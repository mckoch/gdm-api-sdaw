...
class CALENDAR {
    ...
    public function getBelegungForId($id) {
        require_once ('application_2.ini.php');
        $dbi = new DBI();
        $sql = "SELECT STA.count,
            FRE_FREIE_TAFELN.Zeitraum FROM STA 
             JOIN FRE_FREIE_TAFELN ON 
             (STA.Standortnr = FRE_FREIE_TAFELN.Standortnr 
             AND STA.Stellennummer = FRE_FREIE_TAFELN.Stellennr)
             WHERE STA.count  = " . $id . ";";
        return $dbi->arr($sql);
    }
    
    public function getDetailsForTerminId($id) {
        // print $id;
        require_once ('application_2.ini.php');
        $termin = explode('-', $id);
        $dbi = new DBI();
        $dbi->sqldebug(true);
        $sql = "SELECT STA.count, STA.Belegdauerart,
            FRE_FREIE_TAFELN.* FROM STA 
             JOIN FRE_FREIE_TAFELN ON 
             (STA.Standortnr = FRE_FREIE_TAFELN.Standortnr 
             AND STA.Stellennummer = FRE_FREIE_TAFELN.Stellennr 
             AND FRE_FREIE_TAFELN.Zeitraum = ". $termin[1] .")
             
        
             WHERE STA.count  = " . $termin[0] . ";";
        //print $sql;
        return $dbi->arr($sql);
    }
     ...
     * @param type $kw
     * @return array 
     * alle freien Stellen f�r eine KW - summarisch
     */
    ...
    public function kwData($kw, $list) {
        require_once ('application_2.ini.php');
         ...
         ini_set('display_errors', E_ALL); // WARUM AUCH IMMER OHNE GEHT'S GERADE NICHT DZZZZZZZZ 
        $dbi = new DBI();
        $idorloop = '';
        $kworloop = '';
        $dekas = '';
        ...
        foreach ($kw as $item) {
            ...
            $dekas[] = $this->weekToDekas($item);
        }
        //print_r($dekas); die;
        
        if (is_array(@$dekas)) {
            foreach ($dekas as $items) {
                //print_r($items);
                foreach ($items as $item) {
                    //print_r($item);
                    if (is_array($item)) {
                        if (array_key_exists('full', $item)) {
                            $dekaorloop .= " FRE_FREIE_TAFELN.Zeitraum=" . $item['full'] . " OR ";
                        }
                    }
                }
            }
        ...
        $dekaorloop = substr($dekaorloop, 0, -4);
        $sql = "SELECT STA.count AS id, FRE.Belegdauerart,FRE_FREIE_TAFELN.count as TerminId,FRE_FREIE_TAFELN.Tagespreis, 
            ...
            AND STA.Stellennummer=FRE_FREIE_TAFELN.Stellennr ) 
            WHERE ( FRE.Belegdauerart LIKE 'W%' AND (" . $kworloop . ") AND (" . $idorloop . "))
            OR ( FRE.Belegdauerart LIKE 'D%' AND (" . $dekaorloop . ") AND (" . $idorloop . "))LIMIT 10000";
    ...
    public function weeksInDeka($deka){
        $dekadata = $this->getDekaNo(intval($deka));
        print_r($dekadata); die;
    }
    ...
    public function getDekaNo($deka) { 
        return array('dekano' => $deka, 'dekaplan' => $this->dekaplan[intval($deka)]);
    }
