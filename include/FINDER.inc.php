...
class FINDER extends DBI {
    ...
    public function setparams($params) {
        ...
        foreach ($sta as $hs) {
            if ($hs != 'U' && $hs != 'B' && $hs != 'H') {
                ...
                if ($hs == "U")
                    $bel[] = " beleuchtung LIKE 'U' ";
                if ($hs == "B")
                    $bel[] = " beleuchtung LIKE 'B' ";
                if ($hs == "H")
                    $bel[] = " beleuchtung LIKE 'H' ";
                if ($hs == "R")
                    $bel[] = " beleuchtung LIKE 'H' ";
            }
        }
        ...
        //print $this->stafilter; die;
        return true;
    }
    
    public function plzAreaSearch() {  
        ...
        AND PLZ LIKE '" . substr($this->params['value'],0,8) . "' LIMIT 0 , 2000;";

        parent::sqldebug(false);
        return parent::rs($sql);
    }
    
    public function gkzfAreaSearch() {  
        // $lat = $this->params['latitude'];
        // $lng = $this->params['longitude'];
        // $distance = $this->params['umkreis'] / 1000;
        $gfk = explode(',', $_GET['gfk']);
        ...
        $sql = "SELECT * FROM STA WHERE leistungswert1 >= $gfk[0] AND leistungswert1 <=$gfk[1] AND
        preis >=$prc[0] AND preis <=$prc[1] AND " . $this->stafilter . "
        AND StatOrtskz LIKE '" . substr($this->params['value'],0,12) . "' LIMIT 0 , 2000;";
     ...
     * @return type 
     */
    public function getPlzList($plz) {
        $plz = substr($plz,0,8);
        $sql = "SELECT plz99, plzort99 FROM   post_code_areas WHERE ";

        $sql .= " plz99 LIKE '" . $plz . "%' OR plzort99 LIKE '" . $plz . "%' LIMIT 0,20;";
    ...
    public function getOkzfList($okzf){
        $okzf = substr($okzf,0,8);
        $sql = "SELECT Gemeindekennziffer, Ort FROM   GEMEINDE_ZUORDNUNG WHERE ";

        $sql .= " Gemeindekennziffer LIKE '" . $okzf . "%' OR Ort LIKE '" . $okzf 
                . "%' OR PLZ LIKE '" . substr($okzf,0,3) . "%'  LIMIT 0,20;";
     ...
     * @param <array> $data
     * @return <array>
     * @todo: Suchen vers�ubern: SELECT-Auswahlreihenfolge!
     */
...
public function checkoutsinglesysidsearch($id) {
        $sql = "SELECT * FROM STA WHERE  count=" . $id . ";";
        //leistungswert1 >= $gfk[0] AND leistungswert1 <= $gfk[1] AND preis >=$prc[0] AND preis <=$prc[1] AND ".$this->stafilter." AND (";
        
        parent::sqldebug(false);
        return parent::arr($sql);
    }
    ...
    public function gposAreaSearch() {
        ...
        preis >=$prc[0] AND preis <=$prc[1] AND " . $this->stafilter . "
        HAVING distance <= $distance ORDER BY distance LIMIT 0 , 2000;";
